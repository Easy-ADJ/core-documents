# 📋 모빌리티 정산 서비스 상세 기획 (확정)

> 이 프로젝트의 **확정 기획서**다. 무엇을 왜 만드는지에 대한 기준 문서이며, 문제 4 "결제/정산의 멱등성과 정합성"에 대응한다.
> 후보 탐색 과정에서 후보 D로 채택된 경위는 [archive/project-candidates.md](./archive/project-candidates.md) 참조.
>
> ⚠️ **아키텍처(§4)는 일부 현행이 아니다.** 이 문서는 단일 서버 + Amazon RDS를 전제로 작성됐으나, 이후 **서비스 3개 분리 + Supabase PostgreSQL 공유**로 바뀌었다. 아키텍처에 관해서는 [02-design/architecture.md](../02-design/architecture.md)가 단일 출처다. §1~3(문제·시나리오·기능 명세)은 그대로 유효하다.

---

## 1. 문제 재정의

모빌리티 플랫폼은 승차·이용이 끝날 때마다 승객에게 결제를 청구하고, 그 대금을 기사(또는 차량 소유자)에게 주기적으로 정산해 지급한다. 이 흐름에서 실무적으로 반복되는 두 가지 사고 유형이 있다.
...
두 문제 모두 "돈이 걸린 시스템은 재시도해도 결과가 달라지면 안 된다(멱등성)"와 "장부의 합은 항상 맞아야 한다(정합성)"는 동일한 원칙으로 귀결되며, 이는 결제 서비스뿐 아니라 은행·전자지갑 등 핀테크 전반에서 쓰는 **이중기입 원장(double-entry ledger)** 패턴으로 해결한다. ([Double-Entry Ledger Architecture for FinTech](https://medium.com/@gupta.rajneesh2010/20-double-entry-ledger-architecture-for-fintech-50d2ac6eb8e6), [Ledger System Design: Principles for Accuracy, Auditability, and Scale](https://fintechly.com/infrastructure/infrastructure-ledger-system-design/))
...
>
> 팀 리드가 말했다.
> "이번 스프린트 목표는 명확해요. 첫째, 같은 결제 요청이 여러 번 와도 실제로는 딱 한 번만 청구되게 만들 것. 둘째, 정산 배치가 끝나면 '왜 이 금액이 나왔는지'를 누구나 원장만 보고 설명할 수 있게 만들 것."
...
| 기능 | 설명 |
|---|---|
| 결제 요청 API | `POST /api/payments` — 운행(Trip) 종료 시 결제 청구. 요청 헤더에 `Idempotency-Key`(클라이언트가 생성한 UUID)를 필수로 받는다 |
| 멱등 처리 | 동일한 `Idempotency-Key`로 재요청이 오면 실제 결제를 다시 실행하지 않고, **첫 번째 처리 결과를 그대로 반환**한다. DB의 `idempotency_key` 컬럼에 UNIQUE 제약을 걸어 동시 요청이 들어와도 하나만 성공하도록 강제한다 |
...
| 기능 | 설명 |
|---|---|
| 분개 기록 | 결제 승인 시 하나의 거래(Transaction)에 대해 **차변(플랫폼 예수금 증가)** 과 **대변(기사 미지급금 증가)** 두 개의 분개(LedgerEntry)를 원자적으로 함께 기록 |
| 잔액 조회 | `GET /api/ledger/accounts/{accountId}/balance` — 계정(플랫폼/기사별)의 현재 잔액을 분개 합계로 계산 |
| 정합성 검증 | 특정 거래 또는 특정 기간의 모든 분개 합이 정확히 0인지 검증하는 배치/API. 합이 0이 아니면 "정합성 이상" 알림 발생 |
...
| 정산 내역서 생성 | 기사별로 "어떤 운행 건이 얼마로 계산됐는지" 추적 가능한 명세(운행 ID 목록 + 금액)를 정산 항목(SettlementItem)에 남긴다 |
| 재실행 안전성 | 동일 배치 파라미터(정산 대상 일자)로 이미 성공한 Job은 재실행 시 예외를 발생시켜 중복 집계를 막는다. 실패 시에는 실패 지점부터 재시작 |
...
| 불일치 알림 | 대사 결과 불일치 발견 시 로그/알림(간단히는 API 응답 코드나 관리자 대시보드 배지로 시작, 확장 시 SNS 알림 연동) |
...
> ⚠️ **현행 아님.** 아래는 단일 서버 + Amazon RDS 구성이다. 현재는 **결제·원장·정산 3개 서버가 분리되어 서로 API로 호출**하고, DB는 **Supabase PostgreSQL 하나를 공유**한다.
...
> 핵심 설계 원칙: **"잔액을 직접 갱신하지 않고, 분개(LedgerEntry)의 합으로 항상 재계산한다."** 이렇게 하면 특정 시점의 잔액이 이상해도 분개 이력을 replay해서 원인을 추적할 수 있다. ([Double-Entry Ledger Architecture for FinTech](https://medium.com/@gupta.rajneesh2010/20-double-entry-ledger-architecture-for-fintech-50d2ac6eb8e6))

### 4.3 기술 선택과 근거

> ⚠️ **일부 현행 아님.** 아래 표의 **정산 리포트 저장(S3)·배치 스케줄링(EventBridge)** 은 미확정이고, DB는 **Amazon RDS → Supabase PostgreSQL**로 바뀌었다.
...
| 정산 배치 | **Spring Batch** (Chunk 지향 처리) | 대량의 결제 건을 안전하게 나눠 처리하고, 실패 시 실패 지점부터 재시작하는 표준 패턴을 프레임워크가 제공. 카카오페이 정산플랫폼팀도 Spring Batch 기반 정산 처리 성능 최적화 사례를 공개한 바 있다 |
...
> ⚠️ **전제가 깨졌다.** 아래 "결제 생성"은 결제와 원장이 **같은 애플리케이션 안에 있어 하나의 로컬 트랜잭션으로 묶인다**는 전제로 쓰였다. 서버가 분리된 지금은 HTTP 경계를 넘는 트랜잭션이 불가능하다.
> DB는 하나라서 물리적으로는 여전히 가능하지만 서비스 경계가 무너진다 — 선택지와 트레이드오프는 [02-design/architecture.md §3](../02-design/architecture.md#3-결제와-원장의-원자성)에 정리했고, **8/11 회의에서 결정한다.**

- **결제 생성**: `Payment` insert + `LedgerEntry`(차변/대변 2건) insert를 **하나의 DB 트랜잭션**으로 묶어 원자성 보장. `idempotency_key` UNIQUE 제약 위반 시 재요청으로 간주하고 기존 결제 결과를 반환.
- **정산 배치**: Spring Batch의 Chunk 트랜잭션 경계를 활용해 일정 건수(예: 100건)마다 커밋, 실패 시 해당 청크만 롤백 후 재시도.
- **재실행 방지**: 동일 `targetDate` 파라미터로 이미 `CONFIRMED` 상태인 `SettlementBatch`가 있으면 Job 시작 전에 예외 처리.
