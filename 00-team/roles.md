...
서비스 3개를 담당자별로 나눠 개발한다.<br/>
<u>**담당자 = 그 서비스의 API와 테이블에 대한 결정권자**</u>이며, 해당 문서를 수정할 수 있는 유일한 사람이다.
...
> **원장 서버에 병목**이 발생한다.<br/>
> 결제와 정산 **모두** 원장 API에 의존하므로([호출 관계](../02-design/service-contracts.md#1-호출-관계)),<br/>
> 원장 API가 확정되기 전에는 나머지 둘은 목(mock)으로만 진행해야 한다.<br/>
