...
- **절사는 버림(`RoundingMode.FLOOR`), 기사 단위 합계에 한 번만 적용한다.** 결제 건마다 절사하면 건수만큼 오차가 쌓인다. ([공통 규약 §2](../service-contracts.md#2-공통-규약))
...
### `POST /api/settlements/{batchId}/confirm`

`RUNNING` → `CONFIRMED` 전이. **이 호출이 원장에 지급 상쇄 분개를 남긴다** — 기사별로 `POST /api/ledger/entries`를 부르고, 상쇄 금액은 지급액이 아니라 **운임 합계**다.

확정과 상쇄는 **하나의 트랜잭션**이다. 원장 기록이 재시도 후에도 실패하면 배치는 확정되지 않는다. 떼어 놓으면 **미지급금이 남은 채로 확정된 배치**가 생기고, 다음날 배치가 같은 금액을 또 정산한다.

**대사 불일치로 보류된 배치를 사람이 푸는 지점이 여기다.** `MISMATCHED`여도 거부하지 않는다 — 거부하면 보류를 풀 수단이 없어진다. 대신 `reconciliation_status`는 그대로 남아, `CONFIRMED` + `MISMATCHED` 조합이 "사람이 밀어붙인 확정"이라는 증거가 된다.

### `POST /api/settlements/{batchId}/pay`

`CONFIRMED` → `PAID` 전이. 배치와 기사별 `payout_status`를 함께 바꾼다.
**실제 송금은 일어나지 않는다** — 데모 범위에서 `PAID`는 "정산 처리 완료" 표식이다.

> 확정과 지급을 하나로 묶지 않은 이유: `/pay` 하나로 보류까지 풀면 "확정을 거치지 않고 지급된 배치"가 생겨, 나중에 `PAID` 레코드만 보고는 **검증을 통과한 것인지 사람이 밀어붙인 것인지 알 수 없다.**
